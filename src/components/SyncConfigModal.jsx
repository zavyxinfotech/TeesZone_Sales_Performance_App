...
export const SyncConfigModal = ({ onClose }) => {
  ...
  const handleSampleGoogleSheet = () => {
    // Helpful demo guide
    setSheetCsvUrl('https://docs.google.com/spreadsheets/d/e/2PACX-1vT-DEMO-TEESZONE-CSV/pub?output=csv');
  };

  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white border border-outline-variant rounded-2xl max-w-2xl w-full my-8 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="bg-surface-container border-b border-outline-variant/60 p-5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-pink-100 text-brand-pink">
        ...
        <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar text-on-surface">
          {/* Status Message */}
          {syncError && (
            <div className="p-3.5 bg-red-50 border border-red-200 rounded-xl text-xs text-error flex items-start gap-2">
          ...
          {saveSuccess && (
            <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-success flex items-center gap-2">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-xs bg-surface-container border border-outline-variant rounded-lg focus:border-brand-pink focus:ring-1 focus:ring-brand-pink outline-none transition"
                />
              </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-xs bg-surface-container border border-outline-variant rounded-lg focus:border-brand-pink focus:ring-1 focus:ring-brand-pink outline-none transition"
              />
              <p className="text-[11px] text-on-surface-variant mt-1">
            ...
            <div className="flex items-center justify-between pt-2">
              <button
                ...
                className="flex items-center gap-1 text-xs text-on-surface-variant hover:text-error transition"
              >
              ...
              <button
                type="submit"
                disabled={isSyncing}
                className="flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-bold bg-primary-container text-white hover:bg-primary transition shadow-xs disabled:opacity-50"
              >
          ...
          {/* 3-Step Setup Guide */}
          <div className="card-surface-warm p-4 rounded-xl border border-[#E5DED4] space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-primary flex items-center gap-1.5">
              <HelpCircle className="w-4 h-4 text-brand-pink" />
              How to Publish Google Sheet in 3 Simple Steps:
            </h4>
            <ol className="text-xs space-y-2 text-on-surface list-decimal list-inside leading-relaxed">
              <li>
