...
export const TeamTable = ({ onSelectRep }) => {
  ...
  const getStatusBadge = (status, type) => {
    switch (type) {
      case 'success':
        return (
          <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-black bg-gradient-to-b from-[#F0FDF4] to-[#DCFCE7] text-[#15803D] border border-[#86EFAC] shadow-[1px_2px_4px_rgba(21,128,61,0.15)] whitespace-nowrap flex-shrink-0">
      ...
      case 'warning':
        return (
          <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-black bg-gradient-to-b from-[#FFFBEB] to-[#FEF3C7] text-[#B45309] border border-[#FDE68A] shadow-[1px_2px_4px_rgba(180,83,9,0.15)] whitespace-nowrap flex-shrink-0">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-black bg-gradient-to-b from-[#FEF2F2] to-[#FEE2E2] text-[#B91C1C] border border-[#FCA5A5] shadow-[1px_2px_4px_rgba(185,28,28,0.15)] whitespace-nowrap flex-shrink-0">
  ...
  return (
    <div className="skeuo-card overflow-hidden flex flex-col h-full w-full max-w-full">
      {/* Header & Filter Controls */}
      <div className="p-3.5 sm:p-5 md:p-6 border-b border-outline-variant/40 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 w-full max-w-full">
        <div>
          <h3 className="text-sm sm:text-card-heading text-on-surface font-black">
            Team Performance & Pipeline Review
          </h3>
          <p className="text-[10px] sm:text-xs text-on-surface-variant font-medium mt-0.5">
            Individual Breakdown ({reps.length} Active Reps) • Tap to view all 8 questions
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full md:w-auto max-w-full">
          ...
          <div className="relative flex-1 sm:w-52 md:w-60">
            ...
            <input
              ...
              className="skeuo-inset w-full pl-8 pr-3 py-1.5 sm:py-2 text-xs text-on-surface rounded-xl focus:border-brand-pink outline-none font-medium"
            />
          </div>

          {/* Filter Pills */}
          <div className="skeuo-inset flex items-center gap-1 p-0.5 sm:p-1 rounded-xl text-[10px] sm:text-xs font-black overflow-x-auto max-w-full custom-scrollbar flex-nowrap">
            <button
              onClick={() => setFilterStatus('ALL')}
              className={`px-2 sm:px-3 py-1 rounded-lg transition-all cursor-pointer whitespace-nowrap flex-shrink-0 ${
      ...
      {/* MOBILE CARD VIEW (Screens < 768px / 320px screens) */}
      <div className="block md:hidden p-2.5 sm:p-3 space-y-2.5 sm:space-y-3 w-full max-w-full">
          ...
          filteredReps.map((rep) => {
            ...
            return (
              <div
                ...
                className="skeuo-card p-3 sm:p-4 space-y-2.5 cursor-pointer active:scale-98 transition-transform w-full max-w-full overflow-hidden"
              >
                {/* Rep Header & Status */}
                <div className="flex items-center justify-between gap-1.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-b from-[#6E1B2D] to-[#470815] text-white flex items-center justify-center text-xs font-black shadow-xs flex-shrink-0">
                      {rep.avatar || rep.name.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h4 className="font-black text-xs sm:text-sm text-on-surface truncate">{rep.name}</h4>
                      <p className="text-[9px] sm:text-[10px] text-on-surface-variant truncate">{rep.email}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-1.5 sm:gap-2 text-xs">
                  <div className="skeuo-inset p-2 rounded-xl min-w-0">
                    <span className="text-[9px] uppercase font-bold text-on-surface-variant block truncate">Actual Achieved</span>
                    <span className="text-xs sm:text-sm font-black text-[#5A1424] block truncate">{formatINR(rep.actualSales)}</span>
                    <span className="text-[9px] text-on-surface-variant block mt-0.5 truncate">({percentAchieved}% of {formatINR(rep.monthlyTarget, true)})</span>
                  </div>

                  <div className="skeuo-inset p-2 rounded-xl min-w-0">
                    <span className="text-[9px] uppercase font-bold text-on-surface-variant block truncate">Balance Target</span>
                    <span className="text-xs sm:text-sm font-black text-secondary block truncate">{formatINR(balance)}</span>
                    <span className="text-[9px] text-on-surface-variant block mt-0.5 truncate">{rep.activeLeadsCount} Leads</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="skeuo-btn w-full py-1.5 sm:py-2 px-2.5 rounded-xl text-[11px] sm:text-xs font-black text-primary flex items-center justify-center gap-1"
                >
